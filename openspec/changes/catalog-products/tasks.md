...
|------|------|-----------|-------|
| 1 | Mock catalog/detail UI | PR1 | First slice; no Prisma/API/cart/checkout/admin. |
| 2 | Prisma schema and seed | PR2 | Persistence foundation. |
| 3 | Public read API | PR3 | Read-only shopper data. |
| 4 | API-backed UI | PR4 | Preserve Slice 1 component contracts. |
| 5 | Cart | PR5 | Product/variant selections. |
| 6 | Checkout + Mercado Pago | PR6 | Payment flow after cart. |
| 7 | Product admin | PR7 | Admin CRUD separated from public API. |

## Phase 1: Slice 1 - Catalog UI with local mock data + Product Detail UI with local mock data

- [ ] 1.1 Create `src/lib/catalog/types.ts`, `mock-data.ts`, and `queries.ts` with ARS prices, proprietary brands, flat categories, variants, images, and inactive filtering.
- [ ] 1.2 Create `src/app/catalogo/page.tsx` and `src/components/catalog/{CatalogFilters,CatalogTopBar,ProductGrid,ProductCard,CatalogEmptyState}.tsx`.
- [ ] 1.3 Create `src/app/productos/[slug]/page.tsx` and `src/components/catalog/{ProductGallery,VariantSelector,MockCartCTA}.tsx`.
- [ ] 1.4 Update `src/components/layout/header.tsx` and `src/components/home/home-data.ts`; verify lint, typecheck, and manual UI scenarios.
