...
const restoreProduct = vi.hoisted(() => vi.fn());
...
vi.mock("@/app/api/internal/catalog/products/[productId]/restore/route", () => ({ POST: restoreProduct }));
...
describe("admin catalog server action", () => {
  ...
  it("forwards restore through the server action without exposing the CSRF token", async () => {
    headersMock.mockResolvedValue(new Headers({ origin: "https://shop.example", "sec-fetch-site": "same-origin" }));
    cookiesMock.mockResolvedValue({
      get: (name: string) => name === "admin_csrf_token" ? { value: "secret-token" } : undefined,
      getAll: () => [{ name: "admin_csrf_token", value: "secret-token" }, { name: "sb-access-token", value: "session" }],
    });
    restoreProduct.mockResolvedValue(Response.json({ id: "product-1", status: "draft", isActive: true, featured: false }));

    const { mutateAdminCatalog } = await import("@/app/admin/actions");
    const result = await mutateAdminCatalog({ operation: "restore", productId: "product-1" });
    const request = restoreProduct.mock.calls[0][0] as Request;

    expect(result).toEqual({ ok: true, product: { id: "product-1", status: "draft", isActive: true, featured: false } });
    expect(request.headers.get("x-csrf-token")).toBe("secret-token");
    expect(JSON.stringify(result)).not.toContain("secret-token");
  });
});
