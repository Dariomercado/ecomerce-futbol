...
describe("POST /api/internal/orders/[orderId]/refund", () => {
  ...
  it("rejects a human Bearer token without a verified active operator session", async () => {
    ...
    expect(response.status).toBe(403);
    await expect(response.json()).resolves.toEqual({ code: "ADMIN_ACCESS_DENIED" });
    expect(createPrismaAdminAuditRepository).not.toHaveBeenCalled();
    expect(loadPaymentConfig).not.toHaveBeenCalled();
    expect(executePostPaymentOperation).not.toHaveBeenCalled();
  });

  it("denies missing or unavailable operator authorization before payment work", async () => {
    const { POST } = await import("./route");

    for (const authorization of [
      { authorized: false as const, status: 401 as const, code: "ADMIN_SESSION_REQUIRED" as const },
  ...
  it("executes a valid refund and appends attempted then successful audits", async () => {
    configurePayment();
    executePostPaymentOperation.mockResolvedValue({ operation: "refund", status: "completed", provider: { id: "provider-order", status: "refunded" } });
    const { POST } = await import("./route");

    const response = await POST(new Request("http://localhost", { method: "POST" }), { params });

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ operation: "refund", status: "completed", provider: { id: "provider-order", status: "refunded" } });
  ...
  it("records a terminal failure after an invalid refund", async () => {
    configurePayment();
    ...
    expect(appendAudit).toHaveBeenNthCalledWith(2, {
      ...
      entityId: "order-1",
      outcome: "FAILED",
      context: { code: "ORDER_NOT_REFUNDABLE" },
    });
  });

  it("does not report a completed refund when its terminal audit fails", async () => {
    configurePayment();
    executePostPaymentOperation.mockResolvedValue({ operation: "refund", status: "completed", provider: { id: "provider-order", status: "refunded" } });
