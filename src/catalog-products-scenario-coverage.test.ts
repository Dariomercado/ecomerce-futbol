...
const scenarios: readonly ScenarioMap[] = [
  ...
  ["cart / Guest checkout order boundary / Guest order is submitted", "src/app/api/checkout/checkout-routes.test.ts", "persists a created order"],
  ...
  ["catalog-products-domain / First-slice boundary / Backend work is proposed", "src/app/api/catalog/public-api.test.ts", "rejects public mutation attempts"],
  ...
  ["products-public-api / Public read-only catalog APIs / Mutation is requested", "src/app/api/catalog/public-api.test.ts", "rejects public mutation attempts"],
