...
function ImageFields({ availableVariantSkus, fileInputRef, form, onAddFiles, onRemovePreview, setForm }: {
  ...
  return <fieldset className="space-y-3 border-t pt-5"><legend className="text-lg font-semibold">Images</legend>
    ...
    {form.images.map((image, index) => <div className="grid gap-3 rounded-md border p-3 sm:grid-cols-2" key={image.id}>
      {image.file ? <div className="sm:col-span-2"><img alt={`Local preview for ${image.file.name}`} className="h-32 w-32 rounded object-cover" src={image.previewUrl} /><p className="mt-1 text-xs text-muted-foreground">{image.file.name}</p></div> : <Field label={`Image URL ${index + 1}`}><input required type="url" value={image.url} onChange={(event) => updateImage(setForm, index, "url", event.target.value)} /></Field>}
      <Field label={`Alt text ${index + 1}`}><input required value={image.alt} onChange={(event) => updateImage(setForm, index, "alt", event.target.value)} /></Field>
...
const fieldControlClassName = "mt-1 min-h-10 w-full rounded-md border border-border bg-background px-3 py-2 text-foreground shadow-sm outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/30";

function Field({ children, label }: { children: React.ReactNode; label: string }) {
  const control = isValidElement<{ className?: string }>(children)
    ? cloneElement(children, { className: `${fieldControlClassName} ${children.props.className ?? ""}` })
    : children;

  return <label className="grid gap-1 text-sm font-medium">{label}{control}</label>;
}
